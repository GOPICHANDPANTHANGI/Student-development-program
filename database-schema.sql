...
-- Enable RLS (we'll refine when wiring Auth)
...
-- Temporary permissive policies for development (public read, insert)
-- We'll replace these with proper auth-based policies when we wire Auth.
do $$
begin
  ...
  if not exists (select 1 from pg_policies where schemaname='public' and tablename='programs') then
  ...
  if not exists (select 1 from pg_policies where schemaname='public' and tablename='enrollments') then
  ...
  if not exists (select 1 from pg_policies where schemaname='public' and tablename='vote_receipts') then
  ...
  if not exists (select 1 from pg_policies where schemaname='public' and tablename='suggestions') then
